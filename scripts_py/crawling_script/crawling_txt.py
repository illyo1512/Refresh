...
def run_and_save_articles():
    ...
    if not articles:
        print("🚫 새 기사 없음")
        print(json.dumps({"success": True, "hasNewArticles": False, "message": "새 기사 없음"}))
        return

    save_dir = paths.crawling_data_dir / "naver_articles"
    paths.ensure_dir_exists(save_dir)
    ...
    for idx, (url, title) in enumerate(articles):
        ...
        # ✅ 파일 저장
        with open(save_dir / f"{number_part}_url.txt", 'w', encoding='utf-8') as f:
            f.write(url)
        with open(save_dir / f"{number_part}_title.txt", 'w', encoding='utf-8') as f:
            f.write(title)
        with open(save_dir / f"{number_part}_article.txt", 'w', encoding='utf-8') as f:
            f.write(body)

        print(f"✅ 저장 완료: {number_part}_*.txt")

    print(json.dumps({"success": True, "hasNewArticles": True, "message": f"{len(articles)}개의 새 기사 저장 완료", "crawlNum": f"{len(articles)}"}))
